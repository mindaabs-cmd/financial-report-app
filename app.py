...
def suggest_ifrs_category(account_name):
    ...
    elif any(w in name for w in ["قرض طويل", "مكافأة نهاية الخدمة", "مكافا نهاية الخدمة", "التزام طويل", "loan", "end of service", "long term"]):
...
# ==========================================
# 2. إعدادات الشريط الجانبي (مؤتمت بالكامل بدون إدخالات يدوية غير مترابطة)
# ==========================================
...
st.caption(f'التقرير: **{report_type}** لعام **{fiscal_year}** | معايير الإبلاغ المالي الدولية **Full IFRS** (مربوط ومؤتمت بالكامل)')
    ...
    if file:
        ...
        if demo_type == "current":
            d_frame = pd.DataFrame({
                'اسم الحساب': ['النقد بالبنك', 'العملاء', 'المخزون', 'الأثاث والمعدات', 'مجمع الإهلاك', 'الموردون', 'مصاريف مستحقة', 'قرض طويل الأجل', 'رأس المال', 'توزيعات أرباح', 'إيراد المبيعات', 'تكلفة المبيعات', 'رواتب وأجور', 'إيجار المقر', 'مصروف الإهلاك'],
                'مدين': [85000, 140000, 60000, 300000, 0, 0, 0, 0, 0, 10000, 0, 120000, 45000, 15000, 15000],
                'دائن': [0, 0, 0, 0, 15000, 95000, 10000, 100000, 100000, 0, 440000, 0, 0, 0, 0]
            })
        else:
            d_frame = pd.DataFrame({
                'اسم الحساب': ['النقد بالبنك', 'العملاء', 'المخزون', 'الأثاث والمعدات', 'مجمع الإهلاك', 'الموردون', 'مصاريف مستحقة', 'قرض طويل الأجل', 'رأس المال', 'إيراد المبيعات', 'تكلفة المبيعات', 'رواتب وأجور', 'إيجار المقر'],
                'مدين': [50000, 110000, 50000, 250000, 0, 0, 0, 0, 0, 0, 100000, 40000, 12000],
                'دائن': [0, 0, 0, 0, 10000, 80000, 80000, 100000, 100000, 380000, 0, 0, 0]
            })
...
# ==========================================
# الاستخراج الأوتوماتيكي للتسويات (الإهلاك والتوزيعات) من الميزان
# ==========================================
def extract_auto_adjustments(target_df):
    dep_rows = target_df[target_df[account_col].str.contains("إهلاك|اهلاك|depreciation", na=False, case=False)]
    dep_val = dep_rows['Net_Debit_Focus'].sum() if not dep_rows.empty else 15000.0
    
    div_rows = target_df[target_df[account_col].str.contains("توزيعات|مسحوبات|dividends|drawing", na=False, case=False)]
    div_val = div_rows['Net_Debit_Focus'].sum() if not div_rows.empty else 10000.0
    
    return abs(dep_val), abs(div_val)

auto_depreciation, auto_dividends = extract_auto_adjustments(edited_df)

# ==========================================
# 3. حساب وتجهيز القوائم المالية (الحالية والسابقة)
# ==========================================
def calculate_financials(target_df):
    ...
    fallback_cap = 100000.0
    fallback_ret = 20000.0
    
    cap = tb_cap if tb_cap != 0 else fallback_cap
    ret = tb_ret if tb_ret != 0 else fallback_ret
    ret_earn = ret + ni - auto_dividends
    tot_eq = cap + ret_earn
    tot_liab = nc_l + c_l
...
# استخراج أرصدة بداية الفترة أوتوماتيكياً من أرصدة السنة السابقة للمقارنة
beg_cash = prev_cash if uploaded_file_prev else 50000.0
beg_c_assets = prev_c_a_ex if uploaded_file_prev else 120000.0
beg_c_liab = prev_c_l if uploaded_file_prev else 80000.0
beg_nc_assets = prev_nc_a if uploaded_file_prev else 300000.0
beg_nc_liab = prev_nc_l if uploaded_file_prev else 100000.0
...
eq_df = pd.DataFrame({
    ...
    'الأرباح المبقاة (SAR)': [retained_earnings - net_income + auto_dividends, net_income, -auto_dividends, retained_earnings],
    'إجمالي حقوق الملكية (SAR)': [capital + retained_earnings - net_income + auto_dividends, net_income, -auto_dividends, total_equity]
})
...
operating_cf = net_income + auto_depreciation - delta_c_assets + delta_c_liab
investing_cf = -(delta_nc_assets + auto_depreciation)
financing_cf = delta_nc_liab - auto_dividends
...
cf_df = pd.DataFrame({
    'بيان التدفقات النقدية (IAS 7)': [
        'صافي الربح قبل الضريبة والزكاة', 'تعديل: مصروف الإهلاك (أوتوماتيكي)',
        ...
        'النقدية وما في حكمها - بداية الفترة (تلقائي)', 'النقدية وما في حكمها - نهاية الفترة (المحسوبة)'
    ],
    'المبلغ (SAR)': [net_income, auto_depreciation, -delta_c_assets, delta_c_liab, operating_cf, investing_cf, financing_cf, net_cash_change, beg_cash, calculated_ending_cash]
})

# ==========================================
# 4. عرض التبويبات والقوائم المالية
# ==========================================
st.divider()
...
with tab7:
    ...
    st.info(f'💡 القيم المستنتاجة أوتوماتيكياً: مصروف الإهلاك = {auto_depreciation:,.2f} SAR | توزيعات الأرباح = {auto_dividends:,.2f} SAR')
